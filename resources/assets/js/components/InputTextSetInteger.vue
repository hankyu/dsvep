...
export default {
    ...
    methods: {
        validateInput(v) {
            ...
            console.log('InputTextSetInteger validateInput val.match(/^d*$/)', val.match(/^\d*$/))

            return this.validWordNumberLimit(val, 1) && val.match(/^\d*$/) ? true : false
        }
    }
}
</script>