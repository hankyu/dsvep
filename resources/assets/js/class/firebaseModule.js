...
export function firebaseModule() {
    ...
    const PATH_MESSAGE = pathPrefix + '/message',
    ...
    function modifyApplyTeacher(acc) {
        let ref = getRef(PATH_AUDIT_TEACHER)
    ...
    function auditTeacher(acc, reason, success) {
        ...
        let postData = {
            to: acc,
            content: success
                ? `因「${reason}」，審核講師完成, 請至成為講師頁面確認。`
                : `因「${reason}」，審核講師失敗, 請至成為講師頁面確認。`
        }
    ...
    function lessonApplyAudit(acc, l_id) {
        l_id = l_id * 1
        let ref = getRef(PATH_AUDIT_LESSON)
            .orderByChild('lesson_id')
            .equalTo(l_id)
        ...
        return ref.once('value').then(function(snapshot) {
            ...
            snapshot.forEach(childSnapshot => {
                count++
                getRef(PATH_AUDIT_LESSON + '/' + childSnapshot.key).update({
        ...
        return ref.once('value').then(snapshot => {
            ...
            if (acc) {
                postData = {
                    to: acc,
                    content: success
                        ? '課程因「' + reason + '」之考量，已審核通過。請至課程頁面確認。'
                        : '課程因「' + reason + '」之考量，未能審核通過，請至課程頁面確認。'
                }
                sendInsiteSystemMessage(postData)
            }
        })
