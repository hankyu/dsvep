  ...
  _const.prototype =
  {
    ...
    _initialAll: function()
    {
      ...
      if (timestamp > 0)
      {
        //Fundraising
        var fund_label =
          '<div class="lesson-label-fund fundraising">' +
            '<span>募資中</span>' +
          '</div>';
        this._shopping_cart.append(fund_label);
        this._lesson_label_wrapper_mobile.append(fund_label);
      ...
      {
        var top_range = Math.floor(this._top_range) - 100;
        ...
        $(window).on('load', function()
        {
          $(window).bind('scroll', function()
          {
            ...
            if ($this_Top < 50)
            {
              ...
              preview_lesson._preview_tab_bar.removeClass('tab-fixed');
            }

            if ($this_Top < top_range + 176)
            {
              preview_lesson._teacher_avator.removeClass('avatar-fixed');
              preview_lesson._teacher_desc.removeClass('col-sm-offset-4');
            }

            if ($this_Top > top_range)
            {
              preview_lesson._preview_tab_bar.attr('style', width);
              preview_lesson._preview_tab_bar.addClass('tab-fixed');
              $('section').css('padding-top', '76px');
              preview_lesson._teacher_avator.addClass('avatar-fixed');
              preview_lesson._teacher_desc.addClass('col-sm-offset-4');
            }
          }).scroll();
        });
      }

      //Calc The Video Time Length
...
function _tab_switch(id)
{
  ...
  $('section:eq(' + id + ')').slideDown();
  window.scrollTo(0, $('section:eq(' + id + ')').offset().top);
}
