        ...
        _set_browse_list: function(browse_list)
        {
            ...
            $.each(browse_list, function(i, v)
            {
                if (v.times > 5)
                {
                    ...
                    _td_m_id = $('<td />', {'nowrap': 'nowrap', 'text': v.id});
                    _td_ip = $('<td />', {'nowrap': 'nowrap', 'text': v.ip});
                    _td_route = $('<td />', {'nowrap': 'nowrap', 'text': v.route});
                    _td_times = $('<td />', {'nowrap': 'nowrap', 'text': v.times});
                    _td_last_time = $('<td />', {'nowrap': 'nowrap', 'text': v.last});
                    ...
                    $(
                      '<label />',
                      {
                        ...
                        'text': '查看更多',
                        'onclick': '_check_datail(' + v.num + ')'
                    });
                    _tr.append(_td_id);
                    _tr.append(_td_m_id);
                    _tr.append(_td_ip);
                    _tr.append(_td_route);
                    _tr.append(_td_times);
                    _tr.append(_td_last_time);
...
{
    var content = '';
    var data    = browse_overview._browse_data.find((n) => n.num == id);
